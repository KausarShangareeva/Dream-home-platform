...
router.post('/:ownerId/books', async (req, res) => {
  const { title, titleRu, author, language, pages, difficulty, genre } = req.body;
  ...
  const book = await Book.create({
    ownerId: req.params.ownerId, title, titleRu, author, language, pages, difficulty, genre,
...
router.patch('/:ownerId/books/:id', async (req, res) => {
  const update = {};
  ['status', 'difficulty', 'genre', 'days', 'doneDate', 'startDate', 'language', 'pages', 'title', 'titleRu', 'author', 'currentPage'].forEach(f => {
