...
// POST /api/personal/:ownerId/listening  { title, language?, difficulty?, hours }
router.post('/:ownerId/listening', async (req, res) => {
  const { title, language, difficulty, hours } = req.body;
  if (!title || !hours) return res.status(400).json({ error: 'title and hours are required' });
  ...
  const item = await ListeningItem.create({
    ownerId: req.params.ownerId, title, language, difficulty, hours: Number(hours),
...
// PATCH /api/personal/:ownerId/listening/:id  { title?, language?, difficulty?, hours?, status?, doneDate? }
router.patch('/:ownerId/listening/:id', async (req, res) => {
  const update = {};
  ['title', 'language', 'difficulty', 'hours', 'status', 'doneDate'].forEach(f => {
